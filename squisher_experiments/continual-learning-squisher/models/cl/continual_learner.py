...
class ContinualLearner(nn.Module, metaclass=abc.ABCMeta):
    ...
    def __init__(self):
        ...
        self.squisher_log_fidelity = True #-> if recycling the accumulator, also probe the empirical Fisher
                                          #   and print cosine/scale diagnostics
        self.squisher_probe_n = 512       #-> bounded diagnostic probe; independent of EWC's fisher_n
    ...
    def estimate_fisher_from_accumulator(self, dataset, allowed_classes=None):
        '''After completing training on a context, recycle Adam's exp_avg_sq as the importance estimate
        ("Squisher"; Li et al., 2025), instead of estimating the Fisher Information from data.

        The scaling is selected by [self.fisher_source]:
        ...
        [dataset] is only used for its size n (and, if [self.squisher_log_fidelity], for a probe of the
        empirical Fisher to report fidelity diagnostics); [allowed_classes] only affects that probe.'''

        n = len(dataset)
        m = self.train_batch_size
        if self.fisher_source == 'squisher_corrected':
            if m is None or not m < n:
                raise ValueError("'squisher_corrected' needs training batch size m < dataset size n "
                                 "(got m={}, n={})".format(m, n))

        # Harvest the accumulator (and momentum, as a convergence diagnostic) from the optimizer state
        est_fisher_info = {}
        raw_sq_norm = 0.
        exp_avg_sq_norm = 0.
        signal_sq_info = {}
        for gen_params in self.param_list:
            for pname, p in gen_params():
                if not p.requires_grad:
                    continue
                name = pname.replace('.', '__')
                state = self.optimizer.state.get(p, {})
                if 'exp_avg_sq' not in state:
                    raise RuntimeError("no Adam 'exp_avg_sq' state for parameter '{}' -- "
                                       "[fisher_source]='{}' requires the adam optimizer".format(
                                           pname, self.fisher_source))
                v = state['exp_avg_sq'].detach().clone()
                raw_sq_norm += v.pow(2).sum().item()
                step = state['step']
                step = step.item() if torch.is_tensor(step) else step
                beta1, beta2 = self.optimizer.param_groups[0].get('betas', (0.9, 0.999))
        ...
        if self.squisher_log_fidelity:
            probe = self._probe_empirical_fisher(dataset, allowed_classes=allowed_classes)
            flat_probe = torch.cat([probe[name].flatten() for name in sorted(probe)])
            flat_est = torch.cat([est_fisher_info[name].flatten() for name in sorted(est_fisher_info)])
            flat_signal_sq = torch.cat([signal_sq_info[name].flatten() for name in sorted(signal_sq_info)])
            cosine = F.cosine_similarity(flat_probe, flat_est, dim=0).item()
            signal_cosine = F.cosine_similarity(flat_probe, flat_signal_sq, dim=0).item()
            probe_norm = flat_probe.norm().item()
            est_norm = flat_est.norm().item()
            print("SQUISHER-FIDELITY context={} source={} n={} m={} cosine={:.6f} "
                  "norm_est={:.6e} norm_probe={:.6e} norm_ratio={:.6f} raw_v_norm={:.6e} "
                  "exp_avg_norm={:.6e} signal_sq_norm_ratio={:.6f} signal_sq_cosine={:.6f}".format(
                      self.context_count + 1, self.fisher_source, n, m, cosine,
                      est_norm, probe_norm, est_norm / probe_norm if probe_norm > 0 else float('nan'),
                      raw_sq_norm ** 0.5, exp_avg_sq_norm ** 0.5,
                      flat_signal_sq.norm().item() / probe_norm if probe_norm > 0 else float('nan'),
                      signal_cosine))

        # Store new values in the network (same bookkeeping as [estimate_fisher])
        for gen_params in self.param_list:
            for pname, p in gen_params():
                if p.requires_grad:
                    name = pname.replace('.', '__')
                    # -mode (=MAP parameter estimate)
                    self.register_buffer('{}_EWC_prev_context{}'.format(name, self.context_count+1 if self.offline else ""),
                                         p.detach().clone())
                    # -precision (approximated by the recycled, rescaled accumulator)
                    if (not self.offline) and hasattr(self, '{}_EWC_estimated_fisher'.format(name)):
                        existing_values = getattr(self, '{}_EWC_estimated_fisher'.format(name))
                        est_fisher_info[name] += self.gamma * existing_values
                    self.register_buffer(
                        '{}_EWC_estimated_fisher{}'.format(name, self.context_count+1 if self.offline else ""),
                        est_fisher_info[name]
                    )

        # Increase context-count
        self.context_count += 1

    def _probe_empirical_fisher(self, dataset, allowed_classes=None):
        '''Estimate the diagonal empirical Fisher (ground-truth labels, per-sample mean) without touching
        any stored EWC buffers; returns a <dict> of per-parameter tensors. Used for fidelity diagnostics.'''
        probe = {}
        for gen_params in self.param_list:
            for pname, p in gen_params():
                if p.requires_grad:
                    probe[pname.replace('.', '__')] = p.detach().clone().zero_()
        mode = self.training
        self.eval()
        ...
        for index, (x, y) in enumerate(data_loader):
            if self.squisher_probe_n is not None and count >= self.squisher_probe_n:
                break
            x = x.to(self._device())
            ...
            for gen_params in self.param_list:
                for pname, p in gen_params():
                    if p.requires_grad and p.grad is not None:
                        probe[pname.replace('.', '__')] += p.grad.detach() ** 2
            count += x.shape[0]
        probe = {name: v / max(count, 1) for name, v in probe.items()}
