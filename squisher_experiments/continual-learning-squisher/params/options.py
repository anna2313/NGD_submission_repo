...
def add_eval_options(parser, main=False, comparison=False, pretrain=False, compare_replay=False, no_boundaries=False,
    ...
    if main:
        ...
        eval_params.add_argument('--valid-split', type=float, default=0., metavar="FRAC",
                                 help="fraction of test-set to set aside as validation-set (0 = no validation-set)")
        eval_params.add_argument('--split-dir', type=str, default=None, metavar="PATH",
                                 help="dir for saving/loading the test/validation split (default: [data-dir]/splits)")
...
def add_cl_options(parser, main=False, compare_all=False, compare_replay=False, compare_hyper=False,
    ...
    if not compare_replay:
        ...
        if not no_boundaries:
            ...
            param_reg.add_argument('--squisher-probe-n', type=int, default=512,
                                   help="-> Squisher: sample count for fidelity diagnostics only")
