...
from data.load import get_context_set, split_data
...
def run(args, verbose=False):
    ...
    # If requested, set aside part of the test-set as a validation-set. Which data points make up the train-, test-
    # and validation-set is recorded to disk; if such a record is already there, that same split is reloaded instead.
    train_datasets, test_datasets, valid_datasets = split_data(
        train_datasets, test_datasets, valid_split=args.valid_split, seed=args.seed,
        split_dir=args.split_dir if args.split_dir is not None else os.path.join(args.d_dir, 'splits'),
        experiment=args.experiment, scenario=args.scenario, contexts=args.contexts, verbose=verbose,
    )
    ...
    if isinstance(model, ContinualLearner) and use_fisher:
        ...
        # -source of the importance estimate: 'empirical' (estimate from data, as in standard EWC) or
        #  recycle Adam's exp_avg_sq accumulator (Squisher; raw / N-scaled / batch-size-corrected)
        model.fisher_source = getattr(args, 'fisher_source', 'empirical')
        model.train_batch_size = args.batch if hasattr(args, 'batch') else None
        model.squisher_probe_n = getattr(args, 'squisher_probe_n', 512)
