...
from optimizers import (
    EFAdam_memory_efficient,
    ReAdam,
)

from model import MNISTSimpleNet
from asdl_fisher import diag_empirical_fisher_asdl
from utils import compute_fisher_snapshot_memory_efficient, to_json_number
...
def main(args):
    ...
    # Data transforms
    # Transforms convert images to PyTorch tensors and normalize the pixel values
    ...
    test_dataset = torchvision.datasets.MNIST(
        root="./MNIST_experiment/data", train=False, transform=transform, download=True
    )
    ...
    # Model setup
    model = MNISTSimpleNet().to(device)

    # Initialize model parameters
    if args.initialization_type == "normal":
        # Initialize the parameters with standard normal noise
        with torch.no_grad():
    ...
    if optimizer_name == "ReAdam":
        ...
        optimizer = EFAdam_memory_efficient(
            model.parameters(),
            lr=lr,
            betas=tuple(betas),
            batch_size=batch_size,
    ...
    computed_fisher = None  # Initialize here to ensure it's defined for the first snapshot if moving_average_fishers_on is True
    ...
    for epoch in range(epochs):
        ...
        if args.moving_average_fishers_on:
            computed_fisher = compute_fisher_snapshot_memory_efficient(
            ...
            if (epoch + 1) % args.fisher_save_period == 0 or (epoch + 1) == epochs:
                fisher_snapshots.append(
                    compute_fisher_snapshot_memory_efficient(
                        exp_avg_sq=exp_avg_sq,
                        snapshot_epoch=epoch + 1,
                        train_loader=train_loader,
                        model=model,
        ...
        for i, (inputs, targets) in enumerate(train_loader):
            ...
            if isinstance(optimizer, EFAdam_memory_efficient):
                outputs = model(inputs)
                loss = criterion(outputs, targets)
                loss.backward()

                # ASDL's diag Fisher helper clears gradients internally, so cache
                # the batch gradients before calling it and restore them for EFAdam.
                cached_grads = [
                ...
                diag_fisher = diag_empirical_fisher_asdl(
                    model=model, loss_fn=criterion, inputs=inputs, targets=targets
                )

                for param, grad in zip(model.parameters(), cached_grads):
                    param.grad = grad

                # Finally do the step
                optimizer.step(diag_fisher=diag_fisher)
    ...
    if args.plot:
        ...
        # Write out predictions vs true labels for the last batch in the training set
        model.eval()
        with torch.no_grad():
            inputs, targets = inputs.to(device), targets.to(device)
            outputs = model(inputs)
            ...
            print("\nPredicted vs True labels for the last batch:")
    ...
    if args.save_model_parameters:
        model_file_name = f"{args.optimizer_name}_{args.batch_size}_{args.beta2}_{args.np_seed}_model.pt"
        model_path = Path("MNIST_experiment/results") / model_file_name
...
if __name__ == "__main__":
    ...
    if args.name_of_json_output_file == None:
        ...
        file_path = os.path.join(
            "MNIST_experiment/results", args.name_of_json_output_file
        )
