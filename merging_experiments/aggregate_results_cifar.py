"""Validate and aggregate CIFAR-10 Fisher-merging suites.

CIFAR counterpart of aggregate_results.py -- identical logic, imports from
write_sweep_plan_cifar instead of write_sweep_plan, and defaults to the
_cifar-suffixed results/output paths.
"""
...
from merging_experiments.write_sweep_plan_cifar import build_commands
...
def parse_args():
    ...
    parser.add_argument(
        "--output-prefix",
        default=None,
        help="Defaults to review_outputs/merging_experiments/<suite>_cifar.",
    )
...
def write_markdown(path, suite, rows, expected, missing):
    # Discovered from the data rather than hardcoded, so a scheme added or
    # removed later (e.g. squisher_mscaled/empirical_nscaled, or the removal
    # of squisher_nscaled_sum) shows up automatically -- the original
    # aggregate_results.py's hardcoded list went stale after that removal.
    schemes = sorted({
...
def main():
    args = parse_args()
    results_dir = Path(args.results_dir or f"merging_experiments/results/{args.suite}_cifar")
    prefix = Path(args.output_prefix or f"review_outputs/merging_experiments/{args.suite}_cifar")
