"""Validate and aggregate maintained Fisher-merging suites."""
...
from merging_experiments.write_sweep_plan import build_commands
...
def parse_args():
    ...
    parser.add_argument(
        "--output-prefix",
        default=None,
        help="Defaults to review_outputs/merging_experiments/<suite>.",
    )
...
def aggregate(results):
    ...
    for (batch_a, batch_b, shard_a, shard_b), group in sorted(grouped.items()):
        ...
        for name in endpoint_names:
            mean, std = _mean_std(
                payload["endpoint_accuracy"][name] for payload in group
            )
        ...
        for scheme in scheme_names:
            ...
            share_mean, share_std = _mean_std(
                payload["merges"][scheme]["mean_weight_share_model_a"]
...
def main():
    args = parse_args()
    results_dir = Path(
        args.results_dir or f"merging_experiments/results/{args.suite}"
    )
    prefix = Path(
        args.output_prefix
        or f"review_outputs/merging_experiments/{args.suite}"
    )
    results, expected, missing = load_results(
        args.suite, results_dir, args.require_complete
    )
